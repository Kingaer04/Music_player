...
root.geometry("485x700+200+10")  # position of the background the x-axis, y-axis, how far from x-axis and y-axis
...
lower_frame.place(x=0, y=400) # this is where we want to position our buttons

bg = Image.open("bg.jpg")
back_ground = ImageTk.PhotoImage(bg)
Label(root, image=back_ground).place(x=0, y=0, width=485, height=400)

image = Image.open("forward.jpg") # to load in the image
...
Frame_Music.place(x=0, y=515)

button_play = Image.open("playb.jpg")
button_play = button_play.resize((70,50))
play_button = ImageTk.PhotoImage(button_play)

button_stop = Image.open("stopb.jpg")
button_stop = button_stop.resize((70,50))
stop_button = ImageTk.PhotoImage(button_stop)

button_pause = Image.open("pausep.jpg")
button_pause = button_pause.resize((70,50))
pause_button = ImageTk.PhotoImage(button_pause)

button_forward = Image.open("forwardb.jpg")
button_forward = button_forward.resize((70,50))
forward_button = ImageTk.PhotoImage(button_forward)

button_back = Image.open("backwardb.jpg")
button_back = button_back.resize((70, 50))
back_button = ImageTk.PhotoImage(button_back)

Button(root, image=play_button, bg= "#FFFFFF", bd =0, width=60, height =60, command=PlayMusic).place(x=215, y=415)

Button(root, text= "Browse file", width = 59, height = 1, font= ("calibri", 12, "bold"), fg = "Black", bg="#FFFFFF", command=AddMusic).place(x=0, y=480)

Button(root, image=stop_button, bg="#FFFFFF", bd=0, width=80, height=60, command=mixer.music.stop).place(x=120, y=415)

Button(root, image=pause_button, bg = "#FFFFFF", bd=0, width = 80, height=60, command=Pause).place(x=300, y=415)

#Label(root, image=volume_button).place(x=50, y=415)

Button(root, image=forward_button, bg="#FFFFFF", bd=0, width=60, height=60, command=NextMusic).place(x=400, y=415)

Button(root, image=back_button, bg="#FFFFFF", bd=0, width=60, height=60, command=PreviousMusic).place(x=30, y=415)

scroll = Scrollbar(Frame_Music)
playlist = Listbox(Frame_Music, width = 77, font = ("Times new roman", 10), bg ="#333333", fg ="grey", selectbackground= "lightblue", cursor="hand2", bd =0, yscrollcommand= scroll.set)
